...
class StoneIslandProductParserAPI(WebsiteParser):
    ...
    def fetch_data(self,category,locale_dict):
        ...
        try:
            ...
            print(f"This is the current url: {current_url}")
            response = session.get(current_url, headers=headers)
            response.raise_for_status()
            json_data = response.json()
            ...
            while start <= num_products:
                ...
                start+=size
                for product in products:
                    print(f"Current product json: {product}")
                    product_info = {
                        'category': category,
                        'type': self.safe_strip(product.get('type', '')),
                    ...
                    print(f"This is the current product info:\n{product_info}")
                    all_products.append(product_info)
                print(f"This is all of the product data for:\nCategory:{category}\n{all_products}")
            return pd.DataFrame(all_products)
        except Exception as e:
            print(f"An error occurred: {e}")
            return pd.DataFrame()
    ...
    def process_categories(self, categories, locale_dicts):
        ...
        print(f"Complete data saved to {self.output_filename}")
        self.count = len(self.data) - 1




class LoroPianaProductParserAPI(WebsiteParser):
    ##COMPLETE
    def __init__(self,job_id,base_url):
        # Initialize with common base URL and empty DataFrame to accumulate results
        self.base_url = base_url
    ...
    def fetch_data(self,category, base_url,country_code, locale):
        ...
        try:
            current_url=base_url.format(category=category, country_code=country_code, locale=locale, page=0)
            print(f"This is the current_url {current_url}")
            response = session.get(current_url, headers=headers)
            response.raise_for_status()
            json_data = response.json()
            total_pages = json_data.get('pagination', {}).get("numberOfPages",0)
            ...
            for page in range(total_pages):
                ...
                for data in items:
                    print(f"This the json for the currently parsed product\n{data}")
                    product_info = {
                        'code': self.safe_strip(data.get('code', '')),
                        'solrIsFeatured': str(data.get('solrIsFeatured', False)).lower(),
                        'invertedImages': str(data.get('invertedImages', False)).lower(),
                    ...
                    print(f"This the data found for the current product\n{product_info}")
                    all_products.append(product_info)
                print(f"Processed {len(items)} products on Page: {page+1}/{total_pages} for Category: {category} URL: {current_url}")
            print(f"This is all of the product data for:\nCategory:{category}\n{all_products}")
            return pd.DataFrame(all_products)
